...
from keras.layers import concatenate,Input, Dense, Convolution2D, MaxPooling2D, AveragePooling2D, ZeroPadding2D, Dropout, Flatten, merge, Reshape, Activation
...
import UMmercedArchive
from UMmercedArchive import load_batch
...
def load_cifar10_data(img_rows, img_cols):
    ...
    X_valid,Y_valid = Get_dataset(False)
    ...
    X_valid = np.reshape(X_valid, [-1,256,256,3])
...
#    (X_train, Y_train), (X_valid, Y_valid) = tf.keras.datasets.cifar10.load_data()
    ...
    X_train = np.resize(X_train,(-1,299,299,3))
    X_valid = np.resize(X_valid,(-1,299,299,3))
    
    
    
    Y_train = np.squeeze(np.eye(10)[Y_train])
    Y_valid = np.squeeze(np.eye(10)[Y_valid])
