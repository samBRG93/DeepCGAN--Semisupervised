...
def eval_confusion_matrix(labels, predictions):
    with tf.variable_scope("eval_confusion_matrix"):
        con_matrix = tf.confusion_matrix(labels=tf.argmax(labels, 1), predictions=tf.argmax(predictions, 1),
                                         num_classes=10)
...
def get_batch(images, labels, iteration, batch_size=128):
    data_dim = images.shape[0]
    iteration = iteration % (int(data_dim / 128) + 1)

    if (iteration + 1) * batch_size > data_dim:
        return images[(data_dim - batch_size):data_dim], labels[(data_dim - batch_size):data_dim]

    batch = (images[iteration * batch_size:(iteration + 1) * batch_size],
             labels[iteration * batch_size:(iteration + 1) * batch_size])
    return batch
...
def minibatch_discriminator(input, num_kernels=100, kernel_dim=5):
    ...
    print('Matrix size: ', activation)
    diffs = tf.expand_dims(activation, 3) - \
            tf.expand_dims(tf.transpose(activation, [1, 2, 0]), 0)
    abs_diffs = tf.reduce_sum(tf.abs(diffs), 2)
    minibatch_features = tf.reduce_sum(tf.exp(-abs_diffs), 2)

    tensor = tf.concat([input, minibatch_features], axis=1)
...
def save_images(images, it, last=False):
    ...
    for i in range(16):
        ...
        sess.run(fwrite)
        os.rename(r'C:\Users\samuele\\' + name,
                  r'C:\Users\samuele\Desktop\working_gan_thesis\EuroSat_GAN\saved_images\data_iterations\\' + name)
    ...
    if last:
        ...
        for i in range(num_of_samples):
            ...
            sess.run(fwrite)
            os.rename(r'C:\Users\samuele\\' + name,
                      r'C:\Users\samuele\Desktop\working_gan_thesis\EuroSat_GAN\saved_images\data\\' + name)
...
def batch_normalization(name, axes, inputs, is_training=None, stats_iter=None, update_moving_stats=True, fused=True):
    if ((axes == [0, 1, 2]) or (axes == [0, 2])) and fused == True:
        ...
        moving_variance = param(name + '.moving_variance', np.ones(inputs.get_shape()[3], dtype='float32'),
                                trainable=False)
        ...
        def _fused_batch_norm_inference():
            ...
            return tf.nn.batch_normalization(inputs, mean, var, offset[None, :, None, None], scale[None, :, None, None],
                                             1e-5), mean, var
...
def generator_conv(x, c, Reuse=True):
    ...
    with tf.variable_scope("generator_conv", reuse=Reuse):
        ...
        x = batch_normalization('Generator.BN0', [0], x)
        ...
        x = batch_normalization('Generator.BN1', [0, 1, 2], x)
        x = tf.nn.leaky_relu(x)
...
def soft_noisy_loss(real_discriminator, fake_discriminator, BATCH_SIZE=128):
    ...
    g_loss = - tf.reduce_mean(
        tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_discriminator, labels=tf.zeros_like(fake_discriminator)))
...
g_loss += tf.reduce_mean(
    huber_loss(tf.reshape(preprocess(x), [-1, 64 * 64 * 3]), tf.reshape(G_sample, [-1, 64 * 64 * 3]))) * recon_weight
